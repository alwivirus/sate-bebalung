...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $hasTableParam = $request->filled('meja') || $request->filled('table');

        // Jika Pelanggan Scan QR Meja (Memiliki parameter nomor meja)
        if ($hasTableParam) {
            $tableNumber = $request->query('meja', $request->query('table', '01'));
        ...
        // Jika Pengunjung Mengakses Domain Langsung (Katalog Profil Makanan & Minuman Tanpa Harga)
        $categories = Category::with(['menus' => function ($query) {
            $query->where('is_available', true)->orderBy('sort_order', 'asc');
        }])->orderBy('sort_order', 'asc')->get();

        return view('customer.showcase', compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            // Tambahkan fee Rp 500 khusus jika pembayaran via QRIS Online
            $qrisFee = ($paymentMethod === 'online') ? 500 : 0;
            $totalAmount = $subtotalAmount + $qrisFee;
